...
const applyTheme = (primaryColorHslString: string) => {
    const root = document.documentElement;
    const primaryColorHsl = parseHsl(primaryColorHslString);
...
export function DynamicThemeProvider({ category, children }: { category?: FormulaCategory, children: React.ReactNode }) {
    const { settings } = useSettings();

    useEffect(() => {
        const primaryColor = category ? settings.categoryColors[category] : settings.primaryColor;
        applyTheme(primaryColor);
    }, [settings.primaryColor, settings.categoryColors, category]);
