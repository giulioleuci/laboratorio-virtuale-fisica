...
import type { Formula, MeasurementRow, ModeState, ProcessedInput, CalculationResult } from "@/lib/types";
  ...
  inputs.forEach(input => {
    ...
    if (values.length > 0) {
      if (sigmas.length === values.length && sigmas.every(s => s > 0)) {
        ...
        const { wMean } = weightedMean(values, []);
...
export default function CurveFitPageClient({ id }: { id: string }) {
  ...
  const [chartCustomState, setChartCustomState] = useState<Record<string, unknown>>({});
